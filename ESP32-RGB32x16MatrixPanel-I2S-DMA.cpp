...
void RGB32x16MatrixPanel_I2S_DMA::configureDMA()
{
    ...
    while(1) {
        ...
        if (actualRefreshRate > 40) // HACK Hard Coded: Minimum frame rate of 160
          break;
...
void RGB32x16MatrixPanel_I2S_DMA::updateMatrixDMABuffer(int16_t x_coord, int16_t y_coord, uint8_t red, uint8_t green, uint8_t blue)
{
...
void RGB32x16MatrixPanel_I2S_DMA::swapBuffer(boolean copy) {

      i2s_parallel_flip_to_buffer(&I2S1, backbuf_id);
      if(copy)
        memcpy(&(matrixUpdateFrames[backbuf_id].rowdata[0].rowbits[0].data),&(matrixUpdateFrames[1-backbuf_id].rowdata[0].rowbits[0].data),4096);//sizeof(rowBitStruct) * (COLOR_DEPTH_BITS));
      backbuf_id ^=1;
    }
